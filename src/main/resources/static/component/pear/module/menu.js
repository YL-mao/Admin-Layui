layui.define(['table', 'jquery', 'element'], function (exports) {
	...
	function getMiniNavItemSelector(elem) {
		return elem ? "#" + elem + ".pear-nav-mini .layui-nav-item" : ".pear-nav-mini .layui-nav-item";
	}
	...
	pearMenu.prototype.render = function (opt) {
		...
		// 处理高度
		if (opt.elem) {
			$(getIdSelector(opt.elem)).height(option.height)
		}
	...
	/** 异步菜单接口统一按 R 契约取 data 数组。 */
	function unwrapMenuList(result) {
	...
	function requestMenuJson(type, url) {
		var defer = $.Deferred();
		...
		$.ajax({
			...
			success: function (result) {
				defer.resolve(unwrapMenuList(result));
			},
			error: function () {
				defer.resolve([]);
			}
	...
	function getData(url) {
		return requestMenuJson('GET', url);
	}

	function postData(url) {
		return requestMenuJson('POST', url);
	}
		...
		$.each(option.data, function (i, item) {
			...
			if (item.type === 0) {
				...
				content += '<a class="' + className + '" menu-type="' + item.type + '" menu-url="' + item.href + '" menu-id="' +
					item.id +
					'" menu-title="' + item.title + '" is-blank="' + (item.isBlank != null ? item.isBlank : 0) + '" href="' + href + '"  ' + target + '><i class="' + item.icon +
	...
	function createMenuAndControl(option) {
			...
			$.each(item.children, function (i, note) {
				...
				if (note.type == 0) {
					...
					content += '<a ' + target + ' class="' + className + '" menu-type="' + note.type + '" menu-url="' + note.href +
						'" menu-id="' + note.id +
						'" menu-title="' + note.title + '" is-blank="' + (note.isBlank != null ? note.isBlank : 0) + '" href="' + href + '"><i class="' + note.icon +
		...
		$(getIdSelector(option.elem)).html(menu);
		if (!option.control) {
			return;
		}
		$(getIdSelector(option.control)).html(control + "</div>");
		$(getIdSelector(option.control)).append(controlPe);
		$(getIdSelector(option.control) + " .pear-nav-control").on("click", "[pear-id]", function () {
			$(getIdSelector(option.elem)).find(".pear-nav-tree").css({
				display: 'none'
			});
			$(getIdSelector(option.elem)).find(".pear-nav-tree[pear-id='" + $(this).attr("pear-id") + "']").css({
				display: 'block'
			});
			$(getIdSelector(option.control)).find(".pe-title").html($(this).attr("pear-title"));
		...
		if (obj.children != null && obj.children.length > 0) {
			...
			$.each(obj.children, function (i, note) {
				...
				if (note.type == 0) {
					...
					// 创 建 菜 单 结 构
					content += '<a ' + target + ' class="' + className + '" menu-type="' + note.type + '" menu-url="' + note.href +
						'" menu-id="' + note.id + '" menu-title="' + note.title + '" menu-icon="' + note.icon + '" is-blank="' + (note.isBlank != null ? note.isBlank : 0) + '" href="' + href +
						'" ><i class="' + note.icon + '"></i><span>' + note.title + '</span></a>';
				}
				// 加 载 子 项 目 录
				content += loadchild(note);
	...
	function isHoverMenu(b, option) {
		if (b) {
			var navItem = getMiniNavItemSelector(option.elem);
			...
			$(getIdSelector(option.elem) + " .layui-nav-item").off('mouseenter').unbind('mouseleave');
			$(getIdSelector(option.elem) + " dd").off('mouseenter').unbind('mouseleave');
		}
	}
	...
	function rationalizeHeaderControlWidth(option) {
		...
		var $headerControl = $(getIdSelector(option.control));
		...
		$(getIdSelector(option.control) + " .control").css({"width": rationalizeWidth});

		var navobj = $(getIdSelector(option.control) + ' ul.pear-nav-control.pc');
