layui.define(['message', 'layer', 'jquery', 'element', 'yaml', 'form', 'peartab', 'menu', 'frame', 'theme', 'convert', 'fullscreen'],
	function(exports) {
		"use strict";
		...
		const pearAdmin = new function () {
			...
			/** 公告标已读后刷新顶部铃铛未读数据。 */
			this.refreshMessage = function () {
				if (msgInstance && msgInstance.reload) {
					msgInstance.reload();
					element.render('nav');
				}
			}
