		...
		option.fixed = true;
			if (option.iframe) {
				if (window.PEAR_STATIC_HELPER && window.PEAR_IS_STATIC) {
					option.type = 1;
					option.content = '<iframe class="pear-drawer-frame" data-frame-src="' + option.iframe + '" style="width:100%;height:100%;" scrolling="auto" frameborder="0" src="about:blank" allowfullscreen="true"></iframe>';
					option.success = Aspect(option.success, function (layero) {
						var iframe = layero.find("iframe")[0];
						if (iframe) {
							window.PEAR_STATIC_HELPER.applyFrame(iframe, option.iframe);
						}
					});
				} else {
					option.type = 2;
