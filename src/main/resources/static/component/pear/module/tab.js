layui.define(['jquery', 'element'], function(exports) {
	...
	function getTabRootSelector(elem) {
		return elem ? ".layui-tab[lay-filter='" + elem + "']" : ".layui-tab";
	}
	...
	pearTab.prototype.render = function(opt) {
		...
		if (window.PEAR_IS_STATIC) {
			sessionStorage.removeItem(option.elem + "-pear-tab-data");
			sessionStorage.removeItem(option.elem + "-pear-tab-data-current");
		...
		if (option.session) {
			...
			tabData = option.data ? option.data.slice() : [];
		}
			...
			if (window.PEAR_STATIC_HELPER && window.PEAR_IS_STATIC) {
				$("#" + option.elem).find("iframe[data-frame-src]").each(function() {
					window.PEAR_STATIC_HELPER.applyFrame(this, $(this).attr("data-frame-src"));
				});
			}
		...
		element.on('tab(' + this.option.elem + ')', function(data) {
			...
			if (!option.preload) {
				...
				if (!iframeUrl || iframeUrl === "about:blank") {
					...
					if (window.PEAR_STATIC_HELPER && window.PEAR_IS_STATIC) {
						window.PEAR_STATIC_HELPER.applyFrame($iframe[0], iframeUrl);
					} else {
	...
	pearTab.prototype.addTab = function(opt) {
		...
		element.tabAdd(this.option.elem, {
			...
			content: '<iframe id="' + opt.id + '" data-frameid="' + opt.id +
				'" data-frame-src="' + opt.url + '" scrolling="auto" frameborder="0" src="' + getFrameSrc(opt.url) + '" style="width:100%;height:100%;" allowfullscreen="true"></iframe>',
			id: opt.id
		});
		applyStaticFrameById(this.option.elem, opt.id, opt.url);
		tabData.push(opt);
	...
	pearTab.prototype.addTabOnlyByElem = function(elem, opt, time) {
		...
		if ($(".layui-tab[lay-filter='" + elem + "'] .layui-tab-title li[lay-id]").length <= 0) {
			...
			applyStaticFrameById(elem, opt.id, opt.url);
	...
	pearTab.prototype.refresh = function(time) {
		...
		var url = $iframe.attr("data-frame-src") || $iframe.attr("src");
			if (window.PEAR_STATIC_HELPER && window.PEAR_IS_STATIC) {
				window.PEAR_STATIC_HELPER.applyFrame($iframe[0], url);
			} else {
				$iframe.attr("src", url);
			}
	}
	...
	function getFrameSrc(url) {
		...
		function applyStaticFrameById(elem, id, url) {
			if (window.PEAR_STATIC_HELPER && window.PEAR_IS_STATIC) {
				var iframe = $(".layui-tab[lay-filter='" + elem + "'] .layui-tab-content").find("iframe[id='" + id + "']")[0];
				if (iframe) {
					window.PEAR_STATIC_HELPER.applyFrame(iframe, url);
				}
			}
		}
	...
	function createTab(option) {
		...
		var control =
			'<div class="layui-tab-control"><li class="layui-tab-prev layui-icon layui-icon-left"></li><li class="layui-tab-next layui-icon layui-icon-right"></li><li class="layui-tab-tool layui-icon layui-icon-down"><ul class="layui-nav"><li class="layui-nav-item"><a href="javascript:;"></a><dl class="layui-nav-child">';

		// 处 理 选 项 卡 头 部
		var index = 0;
		$.each(option.data, function(i, item) {
			var iframeUrl = item.url;
			var TitleItem = '';
			...
			if (option.index == index) {
				content += '<div class="layui-show layui-tab-item"><iframe id="' + item.id +
					...
					'" data-frame-src="' + iframeUrl + '" src="' + getFrameSrc(iframeUrl) + '" frameborder="no" border="0" marginwidth="0" marginheight="0" style="width: 100%;height: 100%;" allowfullscreen="true"></iframe></div>'
			} else {
				var src = option.preload ? getFrameSrc(iframeUrl) : "about:blank";
				content += '<div class="layui-tab-item"><iframe id="' + item.id + '" data-frameid="' +
					item.id + '" data-frame-src="' + iframeUrl + '" src="' + src + '" frameborder="no" border="0" marginwidth="0" marginheight="0" style="width: 100%;height: 100%;" allowfullscreen="true"></iframe></div>'
			}
			index++;
		});
	...
	function rollPage(d, option) {
		var $tabTitle = $(getTabRootSelector(option.elem) + " .layui-tab-title");
	...
	function mousewheelAndTouchmoveHandler(option) {
		var $bodyTab = $("body " + getTabRootSelector(option.elem) + " .layui-tab-title");
		var $tabTitle = $(getTabRootSelector(option.elem) + " .layui-tab-title");
		if ($tabTitle.length === 0) {
			return;
		}
